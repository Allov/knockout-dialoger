    ...
    return new $.Deferred(function(dfd) {
        try {
            ...
            if (_.find(self.loadedDialogs(), function(d) {
                ...
                var koco.routerPromise = new $.Deferred(function(koco.routerDfd) {
                    try {
                        koco.router._navigateInner(url, koco.routerDfd);
                    } catch (err) {
                        dfd.reject(err);
                    }
                }).promise();
                ...
                koco.routerPromise
                    .then(function(context) {
                        ...
                        if (!anyPageDialogOpened(self)) {
                            self.koco.routerStateBackOrForward = koco.router.koco.routerState.backOrForward;

                            koco.router.koco.routerState.backOrForward = function(state, direction) {
...
Dialoger.prototype.closeInner = function(data, dialog, dfd) {
    ...
    if (!anyPageDialogOpened(self) && self.koco.routerStateBackOrForward) {
        //$(window).off('popstate.dialoger');
        //koco.router.enable();

        koco.router.koco.routerState.backOrForward = self.koco.routerStateBackOrForward;
        self.koco.routerStateBackOrForward = null;
    }
